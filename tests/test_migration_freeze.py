...
def test_old_database_migrates_to_latest(tmp_path):
    ...
    assert applied == list(range(2, mig.MIGRATIONS[-1]["version"] + 1))
    conn = sqlite3.connect(path)
    ...
    from aipd_os.state.migrations import MIGRATIONS
    assert mig.current_version(path) == MIGRATIONS[-1]["version"]
...
def test_v2_v3_era_db_data_preserved(tmp_path):
    ...
    applied = mig.migrate(path)
    assert applied == list(range(3, mig.MIGRATIONS[-1]["version"] + 1))
    conn = sqlite3.connect(path)
    conn.row_factory = sqlite3.Row
...
def test_migration_runner_is_schema_authority(tmp_path):
    ...
    assert versions == list(range(1, mig.MIGRATIONS[-1]["version"] + 1))
